...
function weatherAdvice(outdoorScore: number): string {
  if (outdoorScore >= 0.7) return "Great outdoor weather — parks ranked up";
  if (outdoorScore >= 0.4) return "Mixed conditions — indoor backups ranked";
  return "Stay inside day — indoor picks first";
}
...
const WX_PILL: Record<WxTone, string> = {
  great: "bg-emerald-50 text-emerald-700 border border-emerald-200",
  mixed: "bg-amber-50 text-amber-700 border border-amber-200",
  poor:  "bg-stone-100 text-stone-600 border border-stone-200",
};
...
// ─── Settings drawer ──────────────────────────────────────────────────────
// Uses a real HTML form (POST /api/settings) matching the existing API route.
...
// ─── ContextHeader ────────────────────────────────────────────────────────
// Replaces WeatherBanner + SettingsBar + date strip.
// Left: date headline + weather. Right: age + address chips + edit toggle.
...
export function ContextHeader({
  ...
  // Date headline: "Wed Apr 30"
  const dateLabel = formatInTimeZone(now, config.timezone, "EEE MMM d");

  // Address display: show "Street, City" (first two comma-parts) or default label
  const rawAddr = cfg.home.isDefault
  ...
  if (!wx) {
    // No weather yet — still show date + profile, prompt refresh
    return (
      <div className="pb-4 border-b border-stone-200 mb-5">
        <div className="flex items-start justify-between gap-5">
          <div>
            <p className="text-[10.5px] font-semibold uppercase tracking-widest text-stone-400 mb-1">Today</p>
  ...
  return (
    <div className="pb-4 border-b border-stone-200 mb-5">
      <div className="flex items-start justify-between gap-5">
        ...
        <div>
          <p className="text-[10.5px] font-semibold uppercase tracking-widest text-stone-400 mb-1">Today</p>
          <h1 className="text-2xl font-semibold tracking-tight text-stone-900 flex items-center gap-2 flex-wrap">
            {dateLabel}
            <span className="text-lg font-normal text-stone-500 flex items-center gap-1.5">
              <span>{icon}</span>
              <span>{Math.round(wx.tempF)}°</span>
              <span className="text-stone-400 text-base font-normal">{wx.shortForecast}</span>
            </span>
          </h1>
          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
            <span className={`inline-flex items-center gap-1 text-[11.5px] font-medium px-2.5 py-0.5 rounded-full ${WX_PILL[tone]}`}>
              {advice}
            </span>
            <span className="text-xs text-stone-400">{wx.precipPct}% rain</span>
            {fetchedAt && (
